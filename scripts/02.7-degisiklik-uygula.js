...
async function main() {
  try {
    ...
    // (metin shared/jess-intro.js'te; eski job'lardan kalan alan temizlenir)
    if (questionsData.intro_audio_text !== undefined) {
      delete questionsData.intro_audio_text;
      console.log("SEGMENT 1 Jess selamlama: SABİT alan → questions.json'dan temizlendi");
    }
...
Ekran basligi: ${ekranBasligiDegisti ? "degisti" : "ayni"} / Jess selamlama: sabit / Konu duyurusu: ${konuDuyuruDegisti ? "degisti" : "ayni"} / Jess kapanis: ${jessOutroDegisti ? "degisti" : "ayni"}\n\nYeni onay sayfasi hazirlaniyor...`
    ...
    } else if (APPROVAL === "render_only" && !konuDuyuruDegisti && !jessOutroDegisti) {
